...
export default {
    ...
    methods: {
        ...
        async getItem () {
            let result = await axios.get(`http://localhost:3000/items?id=${this.itemId}&userId=${this.userId}&ResId=${this.ResId}`);
            if(result.status == 200 && result.data.length > 0){
                this.name = result.data[0].name;
                this.description = result.data[0].description;
                this.price = result.data[0].price;
                this.qty = result.data[0].qty;
                this.category = result.data[0].catId;
            }
            else {
                // go to menu
        ...
        async UpdateItem() {
            await axios.put(`http://localhost:3000/items/${this.itemId}` , {
                name: this.name,
                price: this.price,
                qty: this.qty,
